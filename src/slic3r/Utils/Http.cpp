...
void Http::priv::http_perform()
{
	...
	for (int attempt = 0; attempt < total_attempts; ++attempt) {
		...
		if (res == CURLE_OK) {
			...
			if (!retry_http_statuses) break;                           // transport-only retry mode
...
Http& Http::retries(int retry_count, long initial_backoff_ms)
{
	if (p) {
		...
		p->retry_http_statuses      = true;
	}
	return *this;
}

Http& Http::retries_transport(int retry_count, long initial_backoff_ms)
{
	if (p) {
		p->max_retries             = (retry_count < 0) ? 0 : retry_count;
		p->retry_initial_backoff_ms = (initial_backoff_ms < 0) ? 0 : initial_backoff_ms;
		p->retry_http_statuses      = false;
	}
	return *this;
}
