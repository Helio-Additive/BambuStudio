...
public:
	...
	// Enable retry only when curl reports a retryable transport failure. HTTP
	// responses, including 5xx, are not retried. Use this for non-idempotent
	// mutations that do not carry an idempotency key.
	Http& retries_transport(int retry_count, long initial_backoff_ms = 1000);
